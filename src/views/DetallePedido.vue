  ...
  <div>
    <b-row>
      <b-col cols="12" v-if="mesa">
        <b-card v-if="pedido && pedido != null" :header="cardHeader" tag="article" class="m-3 mt-3"
            ...
            <b-col cols="12">
              <b-form-group>
                <b-btn
                  ...
                  class="ml-3 mb-3 float-right" @click.stop="marcarComoPago()" variant="primary">Marcar como
                  pago</b-btn>
                <b-btn v-if="pedido && pedido.editablepedido == 'SI'" class="ml-3 mb-3 float-right"
                  @click.stop="anularPedido()" variant="primary">Anular Pedido</b-btn>
                <b-btn v-if="pedido && pedido.editablepedido == 'SI'" class="ml-3 mb-3 float-right"
                  @click.stop="abrirModalCambiarMesa()" variant="primary">Cambiar de Mesa</b-btn>
                <b-btn
                  v-if="(pedido && pedido.editablepedido == 'SI') || (pedido.descripcionestado == 'FACTURADO' && pedido.descripcionRolSesion != 'MESERO')"
                  class="ml-3 mb-3 float-right" @click.stop="cargarFormulario(null, 'Agregar')" variant="primary">Agregar
                  Producto</b-btn>
                <b-btn v-if="(pedido && pedido.editablepedido == 'SI') && (items && items.length > 0)"
                  class="ml-3 mb-3 float-right" @click.stop="enviarPedido()" variant="primary"><template
                    v-if="(pedido && pedido.idestado > 1)"> Volver a </template>Enviar
                  Pedido</b-btn>
                <b-btn
                  ...
                  class="ml-3 mb-3 float-right" :disabled="!disableBtnFE || isFE" @click.stop="facturarPedido()"
                  variant="primary">Generar Factura</b-btn>
                <b-btn
                  v-if="(pedido && pedido.descripcionestado == 'FACTURADO') || ((pedido && pedido.editablepedido == 'SI') && (items && items.length > 0) && (pedido && pedido.idestado > 1))"
                  class="ml-3 mb-3 float-right" :disabled="disableBtnFE" @click.stop="facturarFEPedido()"
                  variant="primary">Generar
                  FE</b-btn>
              </b-form-group>
            </b-col>
          </b-row>
          ...
          <b-row align-h="center"
            ...
            <b-col class="contenedor-tabla">
              ...
              <b-form-checkbox id="checkbox-0" v-model="pedido.checkFe" name="checkbox-0" :value="true"
                :unchecked-value="false" :disabled="isFE" class="mb-3">
                ¿Desea generar factura electrónica?
              </b-form-checkbox>

              <b-card title="" tag="article" class="mb-4" v-if="pedido.checkFe">
                <b-container>
                  <b-row>
                    <b-col>
                    ...
                    <b-col>
                      <b-form-group label="Tipo de identificacion">
                        <b-form-select v-if="pedido" :options="tiposDocumento"
                          v-model="clienteFE.identificationObject.type" class="mb-3">
                        </b-form-select>
                      </b-form-group>
                    </b-col>
                    <b-col>
                      <b-form-group description="Si es NIT, no es necesario poner el DV."
                        label="Número de identificación">

                        <b-input-group class="">
                          <b-form-input v-model="clienteFE.identificationObject.number" trim></b-form-input>
                          <b-input-group-append>
                            <b-button variant="primary" :disabled="clienteFE.identificationObject.number.trim() == ''"
                              @click="getClientAlegra()">Buscar</b-button>
                          </b-input-group-append>
                        </b-input-group>
                      </b-form-group>
                    </b-col>
                  ...
                  <b-row v-else>
                    <b-col>
                      <b-form-group label="Razón social / nombre completo">
                  ...
                  <b-row>
                    <b-col>
                      <b-form-group label="Municipio">
                        <b-form-select v-if="pedido" :options="municipios" v-model="clienteFE.address.city"
                          class="mb-3">
                        </b-form-select>
                      </b-form-group>
                    </b-col>
            ...
            <b-col class="contenedor-tabla">
              ...
              <b-card title="" tag="article" class="mb-2">
                ...
                <b-container>
                  ...
                  <b-form-checkbox id="checkbox-2" v-model="asociarCliente" name="checkbox-2" value="true"
                    unchecked-value="false" :disabled="isFE" class="mb-3">
                  ...
                  <template v-if="asociarCliente == 'true'">
                    <b-row class="mb-3">
                      <b-col class="text-left" sm="6">
                        ...
                        <b-form-input type="number" required v-model="pedido.telefonocliente" class="form-control" />
                      </b-col>

                      <b-col sm="6" class="text-left sm-6">
                        <span class="font-weight-bold">
                          <span style="color:red;">* </span>Nombre
                        </span>:<br>
                        <b-form-input type="text" required v-model="pedido.nombrecliente" class="form-control" />
                      </b-col>

                      <b-col sm="12" class="text-left sm-12">
                        <span class="font-weight-bold">
                          <span style="color:red;">* </span>Dirección
                        </span>:<br>
                        <b-form-textarea id="textarea" v-model="pedido.direccioncliente" rows="3"
              ...
              <b-table v-if="items && items.length > 0" stacked="sm" striped hover class="columna-centrada"
                tbody-class="columna-centrada" :fields="campos" :items="items">
                <template slot="acciones" slot-scope="row">
                  <b-button style="margin: 1px;" class="ml-2" @click.stop="cargarFormulario(row.item, 'Ver')">
                    <i class="icon-eye"></i>
                  </b-button>
                  <b-button
                    v-if="(pedido && pedido.editablepedido == 'SI') || (pedido.descripcionestado == 'FACTURADO' && pedido.descripcionRolSesion != 'MESERO')"
                    style="margin: 1px;" class="ml-2" variant="primary"
                    @click.stop="cargarFormulario(row.item, 'Modificar')">
                    <i class="icon-pencil"></i>
                  </b-button>
                  <b-button
                    v-if="(pedido && pedido.editablepedido == 'SI') || (pedido.descripcionestado == 'FACTURADO' && pedido.descripcionRolSesion != 'MESERO')"
                    style="margin: 1px;" class="ml-2" variant="danger"
              ...
              <b-row v-else class="mt-1 mb-2 text-center">
                <b-col cols="12">
                  <b-alert show>El pedido está creado, pero no se encontraron registros de productos pedidos. Haz click
                    en el
                    boton Nuevo para agregar productos.</b-alert>
                  <b-btn class="ml-3 float-right" @click.stop="anularPedido()" variant="primary">Anular Pedido</b-btn>
                </b-col>
              </b-row>
            </b-col>
          </b-row>
        ...
        <b-card v-else :header="cardHeader" tag="article" class="m-3 mt-3" border-variant="primary"
          header-bg-variant="primary">
          <b-row class="mt-1 mb-2 text-center">
            <b-col cols="12">
              <b-alert show variant="info">No ha creado el pedido. Haz click en el boton Crear Pedido para
                iniciar.</b-alert>
              <b-btn class="ml-3 float-right" @click.stop="crearPedido()" variant="primary">Crear Pedido</b-btn>
            </b-col>
          </b-row>
    ...
    <b-modal v-if="objeto" centered v-model="showModal" :title="tipoOperacion" size="lg">
      <b-container>
        <b-form-checkbox v-model="isPizza" name="checkbox-is-pizza" :v-model="isPizza" :unchecked-value="false"
        ...
        <template v-if="isPizza">
          <b-form-group label="Tamaño">
            <b-form-radio-group id="radio-group-size" v-model="sizePizza" name="radio-size-component"
          ...
          <b-form-group label="Fracciones">
            <b-form-radio-group id="radio-group-fraccion" v-model="fraccionPizza" name="radio-fraccion-component"
        ...
        <b-row class="mb-3">
          <b-col cols="6" class="text-left">
            ...
            <b-form-select v-else v-model="objeto.idtipoproducto" class="mb-3">
              <option :key="'tipo_' + t" v-for="(tipo, t) in filtrarTipoProductos(tipoProductos)" :value="tipo.id">
                {{ tipo.descripcion }}</option>
            </b-form-select>
          </b-col>
          <b-col v-if="fraccionPizza == 'MITAD' && isPizza" cols="6" class="text-left">
            <span class="font-weight-bold">
              <span style="color:red;"> </span>Tipo Producto
            </span>:<br>
            <span v-if="(tipoOperacion === 'Ver' || tipoOperacion === 'Eliminar')">
            ...
            <b-form-select v-else v-model="objeto.idtipoproducto2" class="mb-3">
              <option :key="'tipo2_' + t" v-for="(tipo, t) in filtrarTipoProductos(tipoProductos)" :value="tipo.id">
                {{ tipo.descripcion }}</option>
            </b-form-select>
          </b-col>
        </b-row>
              ...
              <template v-for="(tipo, t) in tipoProductos">
                <template v-if="objeto && objeto.idtipoproducto">
                  <template v-for="(producto, p) in tipo.productos">
                    <option v-if="producto.idtipoproducto == objeto.idtipoproducto" :key="'prod_' + t + '_' + p"
                      :value="producto.id">{{ producto.descripcion }}</option>
                  </template>
                </template>
                <template v-else>
                  <template v-for="(producto, p) in tipo.productos">
            ...
            <b-form-select v-else v-model="objeto.idproducto2" class="mb-3">
              <template v-for="(tipo, t) in tipoProductos">
                <template v-if="objeto && objeto.idtipoproducto2">
                  <template v-for="(producto, p) in tipo.productos">
                    <option v-if="producto.idtipoproducto == objeto.idtipoproducto2" :key="'prod2_' + t + '_' + p"
                      :value="producto.id">{{ producto.descripcion }}</option>
                  </template>
                </template>
                <template v-else>
        ...
        <b-row class="mb-3">
          <b-col cols="6" class="text-left">
            ...
            <b-form-select v-else v-model="objeto.cantidadproducto" :options="options">
            </b-form-select>
          </b-col>
        </b-row>
        <b-row class="mb-3">
          <b-col class="text-left">
            ...
            <b-form-input v-else type="text" required v-model="objeto.descripcion" class="form-control" />
          </b-col>
        </b-row>
      </b-container>
      <div slot="modal-footer" v-if="objeto !== null" class="pull-right">
        <b-btn v-if="tipoOperacion === 'Ver'" size="sm" variant="secondary"
          @click.stop="cerrarFormulario">Cerrar</b-btn>
        <b-btn v-if="tipoOperacion === 'Agregar'" size="sm" variant="primary" @click.stop="guardar">Guardar</b-btn>
        <b-btn v-if="tipoOperacion === 'Modificar'" size="sm" variant="primary" @click.stop="guardarCambios">Guardar
          Cambios</b-btn>
        <b-btn v-if="tipoOperacion === 'Eliminar'" size="sm" variant="danger" @click.stop="eliminar">Eliminar</b-btn>
      </div>
    </b-modal>
    ...
    <b-modal centered v-model="showModalCambiarMesa" title="Cambiar de Mesa">
          ...
          <b-col class="text-left">
            ...
            <b-form-select v-model="idMesaNueva" class="mb-3">
              <template v-for="(mesa, m) in mesasDisponibles">
                <option :key="'mesa_' + m" :value="mesa.id" v-if="mesa.descripcionestadopedido == null">
                  {{ mesa.descripcion }}</option>
              </template>
            </b-form-select>
          </b-col>
        </b-row>
      ...
      <div slot="modal-footer" class="pull-right">
        <b-btn variant="primary" @click.stop="cambiarMesa">Cambiar de Mesa</b-btn>
      </div>
    </b-modal>
...
export default {
  ...
  methods: {
        ...
        .confirmWithTitle(
          ...
          function () {
            ...
            self.$http.put("ws/pedido/", frm).then(resp => {
              ...
              self.$toast.success(respuesta.mensaje);
              self.$loader.close();
              self.atras();
            }).catch(resp => {
              self.$loader.close();
            ...
            self.$http.put("ws/pedido/", frm).then(resp => {
              ...
              const itemsValid = self.items.filter((item) => {
                return (
                  ...
                  || item.descripciontipoproducto.includes(self.itemsPrint[11])
                )
              });
    ...
    generarTicketFacturaFE: function () {
      var self = this;
      var token = window.localStorage.getItem("token");
      var frm = {
        productos: self.items,
      ...
      self.$loader.open({ message: "Generando ..." });
      self.$http.post("ws/ticket/facturaFE.php", frm).then(resp => {
        self.$loader.close();
        // self.$toast.success(resp.data.mensaje);
        // self.$toast.success("Exito");
      }).catch(resp => {
    ...
    totalPedido: function () {
      ...
      const totalResponse = '$' + Number(total.toFixed(1)).toLocaleString();
